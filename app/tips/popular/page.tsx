...
export const metadata: Metadata = {
  title: 'Popular Life Hacks - LifeHacking',
  description: 'Explore the most popular life hacks on LifeHacking. Find top-rated tips for cooking, cleaning, productivity, and everyday living.',
 ...
 * 
 * Server component that displays the most popular tips.
 * - Fetches popular tips sorted by creation date with caching (5-minute cache)
 * - Note: API doesn't support sorting by view count, using CreatedAt instead
 * - Uses connection() to defer rendering to request time
 */
export default async function PopularTipsPage() {
  // Defer to request time to avoid build-time API dependency
  await connection();

  // Fetch popular tips with caching
  const response = await getCachedPopularTips(1, 20);
